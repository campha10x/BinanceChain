...
public class Test: WebSocketDelegate {
    ...
    private var address: String {
        return "tbnb10a6kkxlf823w9lwr6l9hzw4uyphcw7qzrud5rr"
    }
    private var addressTwo: String {
        return "tbnb10a6kkxlf823w9lwr6l9hzw4uyphcw7qzrud5rr"
    }
    private var symbol: String {
         return "BNB_BTC.B-918"
    }
    private var hashId: String {
        return "5CAA5E0C6266B3BB6D66C00282DFA0A6A2F9F5A705E6D9049F619B63E1BE43FF"
    }
    private var orderId: String {
        return "7F756B1BE93AA2E2FDC3D7CB713ABC206F877802"
    }
    private var amount: Double = 200
    private var mnemonic: String = "3250883a22016024627f7b371a22b886ffd64bb9d95487d5d866d84db8f0197c"
    ...
    public func runTestsOnTestnet(_ which: Tests) {
        ...
        case .all:
            self.testAPI(endpoint: .mainnet) {
                self.testWallet(endpoint: .mainnet)
                self.testBroadcast(endpoint: .mainnet)
                self.testWebSocket(endpoint: .testnet)
                self.testNodeRPC(endpoint: .mainnet)
            }
        ...
        case .broadcastcontrol:
            self.testBroadcastControl(endpoint: .mainnet)
    ...
    public func testAPI(endpoint: BinanceChain.Endpoint = .mainnet, completion: @escaping ()->()) {
    ...
    public func testBroadcast(endpoint: BinanceChain.Endpoint = .mainnet) {
    ...
    public func testBroadcastControl(endpoint: BinanceChain.Endpoint = .mainnet) {

        // Run a broadcast control test
        
        let binance = BinanceChain(endpoint: endpoint)
        let wallet = Wallet(mnemonic: "3250883a22016024627f7b371a22b886ffd64bb9d95487d5d866d84db8f0197c", endpoint: .mainnet)
        wallet.synchronise() { (error) in

            self.output("wallet.init", wallet, error)

            let symbol = "BET-844_BNB"
    ...
    private func testNodeRPC(endpoint: BinanceChain.Endpoint) {
        ...
        let noderpc = NodeRPC()
        noderpc.connect(endpoint: .mainnet) { (error) in
